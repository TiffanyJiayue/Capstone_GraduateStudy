...
date: "2024-03-12"
---
# NOTE: NEW CODE STARTS FROM ROW 267
# OBJECTIVE: 
1. Check if there are any numbers other than 0 in the Event_Frequency column.
2. Change all the non-zero value in the event frequency column to 1. (Binary)
3. Create binary col for scorecard column.
4. Add company category to mem_data.
5. Plot how many current members in each category.
6. Focus on a specific type category company to make some analysis.
7. Make a plot of "Behavior Comparison between First Six Months and Second Six Months."
8. Add confidence intervals for each bar.
9. Plot how much percentage of Member Usage Before Leaving PTC. ("Event_Participation", "Benefit_Usage", "SC.BusinessDevelopment", "SC.Outreach", "SC.Visibility", "SC.Talent", "SC.Advocacy", "SC.Engagement", "SC.Benefits")
10. Comparison of Event Participation (First Year vs Last Year).
11. Different plots to see which one works better.
12. Run a survival model, ask prof later to improve.
...
cross_data <- "/Users/XXX/Cross-Event_40_Events_Sales_Summary_Excel_93284519469_20240201_2114.xlsx"
...
Cleaned_eventdate <- read.csv("/Users/XXX/CleanedMember.EventParticipation.csv")
```
...
#excel_file_path <- "/Users/XXX/mem_data.xlsx"
...
#### after creating binary col for scorecard, I find that our mem_data has mistake,(a lot of replicating rows, for example Company Name'3Cloud', '4th Source'......) #### ( I didn't realize this issue until I trying to figure out what is the meaning of Payment.Amount, and the month value of those companies are repeated twice.  ####

#### for creating binary col, i tried to create cols such as SC.BusinessDevelopment, SC.Outreach...in scorecard data and then merge to mem_data, but a lot of duplicate cols. Therefore, I do each type separately.
...
# 2256 obs
###### some repeat values appear
###### for example 	
###### 4CTechnologies (Month 45 appears twice, probably because type Business Development has different reasoning)
...
```{r}
category_data <- read.csv("/Users/XXX/company_catergory.csv")
```
```{r}
...
```{r}
member_data <- "/Users/XXX/Current.Members.JoinDate.xlsx"
...
##### survival model ######
##### # Just trying run a model first, discuss and ask prof for more details ######
```{r}
