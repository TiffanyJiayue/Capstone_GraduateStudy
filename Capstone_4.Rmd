...
# OBJECTIVE: 
1. Preparing CrossEvent dataset (Date Format)
2. Preparing MembeParticipation dataset
3. Merge two datasets: Cross Event + CleanedMember.EventParticipation 
4. The new dataset has all the event dates get from Cross Event dataset and CleanedMember.EventParticipation dataset (AllEventDate.csv)
5. Preparing  Attendee data
6. Merge event date to the event att dataset based on "Title"
...
```{r}
mem_data <- read.csv("/Users/XXX/mem_data.csv")
```
...
```{r}
# this dataset is the cleaned dataset with "Company" "Title" (Event Name), "Paid Through" (Paid Through Date), "Company Member Type", "Event.Date" (Event Date)
write.csv(mem_data, file = "mem_data_new.csv", row.names = FALSE)
```
