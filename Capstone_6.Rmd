...
---
Steps:
# 1) filter the dataset only to include the obs that member_status start from 1 to 0
# 2) Create the new variable named "Duration" to count how many member_status = 1
# 3) aggregate all the variables (Event_Participation + Benefit_Usage + SC.BusinessDevelopment + SC.Outreach + SC.Visibility + SC.Talent +SC.Advocacy + SC.Engagement + SC.Benefits + SC.General) 
# (How was the service used in the membership duration time?)

```{r}
final_data <- read.csv("/Users/XXX/final_data_withcate.csv")
```
...
```{r}
cox_model <- coxph(Surv(Duration, Member_Status) ~ Event_Participation + 
                      SC.BusinessDevelopment + SC.Talent + SC.Outreach + SC.Visibility + SC.Advocacy , 
                   data = survival_data)
summary(cox_model)
```
